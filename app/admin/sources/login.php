          ...
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-left py-5 px-4 px-sm-5">
              <div class="brand-logo">
                <img src="https://samdefi.cl/logo.png" alt="logo">
              </div>
              ...
              if($FormBTN == "admin") {
                  ...
                  $checkUser = $db->query("SELECT * FROM ce_users WHERE username='$username'");
                  if(empty($username)) {
                      echo aerror("Please enter your username.");
                  } else {
                      $login = $checkUser->fetch_assoc();
                      if(password_verify($password, $login['password'])) {
                        if($login['level'] == "1" or $login['level'] == "2") {
                            $_SESSION['ce_admin_uid'] = $login['id'];
                            header("Location: ./");
                        } else {
              ...
              <form class="pt-3" action="" method="POST">
                ...
                <div class="mt-3">
                  <button type="submit" name="ce_login" value="admin" class="btn btn-block btn-success btn-lg font-weight-medium auth-form-btn">LOGIN</button>
                </div>
              </form>
            </div>
          </div>
